...
//                String webRootPath = currentPath.substring(0, currentPath.length() - 16);
            	if(SiteMapType.XML.getName().equalsIgnoreCase(YiDuConstants.yiduConf.getString(YiDuConfig.SITEMAP_TYPE))) {
	 ...
	 * @throws Exception
	 */
	public void createBaiduXmlSiteMap(List<TArticle> articleList, List<TChapter> chapterList, String siteMapDir, int fileIndex) throws Exception{
	...
	private String constructURL(TChapter chapter) {
		...
		loc = loc.replace("{sub_dir}", String.valueOf(subDir))
				 .replace("{article_no}", String.valueOf(articleNo))
	 ...
	 * @throws Exception
	 */
	/*public void createBaiduXmlSiteMap2(List<TChapter> chapterList, String siteMapDir, int fileIndex) throws Exception{
		...
		for(TChapter chapter : chapterList) {
			...
			loc.setText(createURL(chapter, URL_TYPE.ARTICLE));
			Element lastmod = url.addElement("lastmod");
			lastmod.setText(DateUtils.format(chapter.getPostdate()));
		...
		TChapter chapter = chapterList.get(chapterList.size());
		Element url = root.addElement("url");
		Element loc = url.addElement("loc");
		loc.setText(createURL(chapter, URL_TYPE.CHAPTER));
		Element lastmod = url.addElement("lastmod");
		lastmod.setText(DateUtils.format(chapter.getPostdate()));
		Element changefreq = url.addElement("changefreq");
		changefreq.setText("always");
		...
		File outFile = new File(siteMapDir+"/sitemap_baidu_"+fileIndex+".xml");
		//百度xml格式的sitemap每个文件最大支持10M， 这里定义文件大小为5M
		//最多支持5万条url
		if(outFile.exists() && outFile.length() < 5 * 1024 * 1024) {
