...
@Results(value = {
        @Result(name = "api/book/download", type = "stream", params = { "inputName", "is", "contentType",
                "application/octet-stream; charset=UTF-8", "contentLength", "${length}", "contentDisposition",
                "attachment; filename =${downloadFileName}" })})
public class DownloadAction extends JsonBasePublicAction {
    /**
     * 串行化版本统一标识符
     */
    ...
    protected ResponseBean<?> loadJsonData() {
        ...
        if (articleno == 0) {
            ...
            responseBean.setDataObj(getText("errors.not.exsits.article"));
            return responseBean;
        }
